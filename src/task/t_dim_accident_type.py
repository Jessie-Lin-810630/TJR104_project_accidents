...
def t_dim_accident_type(csvfile_paths: list[str]) -> pd.DataFrame:
    ...
    for file_path in csvfile_paths:
        ...
        # 擷取需要的欄位
        required_columns = [k for k in dim_accident_type_col_map.keys()]
        df = df.loc[:, required_columns]

        # 重新命名欄位
        renamed_required_columns = [dim_accident_type_col_map[k] for k in required_columns]
        ...
        # 去重
        single_df = df.drop_duplicates(subset=["accident_category",
    ...
    df_dim_accident_type = pd.concat(all_df).drop_duplicates(subset=["accident_category",
                                                                     "accident_position_major",
                                                                     "accident_position_minor",
                                                                     "accident_type_major",
                                                                     "accident_type_minor"])
...
df_dim_accident_type = t_dim_accident_type(csvfile_paths=["/Users/little_po/Desktop/Project/04_Traffic_accidents/taiwan_traffic_accidents/test/processed_data/114年度A1交通事故資料.csv",
